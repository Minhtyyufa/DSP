...
line([f_p1, f_p2], [0, 0],'Color', 'red', 'LineStyle', '--');
line([f_p1, f_p2], [-r_p, -r_p],'Color', 'red','LineStyle', '--');
line([0,f_s1], [-r_s,-r_s],'Color', 'red','LineStyle', '--');
line([f_s2, f_end], [-r_s,-r_s],'Color', 'red','LineStyle', '--');
xlim([0, f_end])
xticks([f_s1, f_p1, f_p2, f_s2])
xticklabels({['fs_1 = ', num2str(round(f_s1/1e6, 1))], ['fp_1 = ', num2str(round(f_p1/1e6, 1))], ['fp_2 = ', num2str(round(f_p2/1e6, 1))], ['fs_2 = ', num2str(round(f_s2/1e6, 1))]})
...
%%  (a)
figure
%Normalized digital radian freq
subplot(3,1,1)
plotspecs([wn_s1, wn_s2]*pi*1e6, [wn_p1, wn_p2]*pi*1e6, wn_end*pi*1e6, r_s, r_p);
title('Specs for normalized analog radian scale')
xlabel('\omega (rad)')
xticks([wn_s1, wn_p1, wn_p2, wn_s2]*pi)
xticklabels({['Ws_1 = ', num2str(round(wn_s1*pi, 2))], ['Wp_1 = ', num2str(round(wn_p1*pi, 2))], ['Wp_2 = ', num2str(round(wn_p2*pi, 2))], ['Ws_2 = ', num2str(round(wn_s2*pi, 2))]})
ylim([-(abs(r_s)+1), 1])
xlim([0, wn_end*pi])
...
om_p1 = prwarp(wn_p1);
om_p2 = prwarp(wn_p2);
%om_end = prwarp(wn_end);       %2*pi*3Mhz/6Mhz = pi => tan(w_end) -> inf, 

subplot(3,1,2)
%plotspecs([om_s1, om_s2]*1e6, [om_p1, om_p2]*1e6, om_end, r_s, r_p);
line([om_p1, om_p2], [0, 0],'Color', 'red', 'LineStyle', '--');
line([om_p1, om_p2], [-r_p, -r_p],'Color', 'red','LineStyle', '--');
line([0,om_s1], [-r_s,-r_s],'Color', 'red','LineStyle', '--');
line([om_s2, om_s2+om_s1], [-r_s,-r_s],'Color', 'red','LineStyle', '--');
title('Specs for normalized analog radian scale')
xlabel('\Omega^p (rad/s)')
xticks([om_s1, om_p1, om_p2, om_s2])
xticklabels({['\Omega_s_1 = ', num2str(round(om_s1, 2))], ['\Omega_p_1 = ', num2str(round(om_p1, 2))], ['\Omega_p_2 = ', num2str(round(om_p2, 2))], ['\Omega_s_2 = ', num2str(round(om_s2, 2))]})
ylim([-(abs(r_s)+1), 1])
xlim([0, om_s2+om_s1])

%
Bn = om_p2 - om_p1;
wn_o = sqrt(om_p2 * om_p1);
wn_p = w2wproto_BP(om_p1, wn_o, Bn);
wn_s_lp1 = w2wproto_BP(om_s1, wn_o, Bn);
wn_s_lp2 = w2wproto_BP(om_s2, wn_o, Bn);
%
subplot(3,1,3)
...
xticklabels({'\Omega_p = 1', ['\Omega_s_1 = ', num2str(min(wn_s_lp1, wn_s_lp2))], ['\Omega_s_2', num2str(max(wn_s_lp1, wn_s_lp2))]})
xlim([0,pi])
xlabel('\Omega^p_L_P (rad/s)')
